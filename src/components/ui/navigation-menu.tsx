...
function NavSmallItem({ item, className, ...props }: React.ComponentProps<typeof NavigationMenuLink> & { item: Omit<NavItemType, 'description'> }) {
  return (
...
function NavItemMobile({ item, className, ...props }: React.ComponentProps<'a'> & { item: NavItemType }) {
  return (
    <a
      ...
      {...props}
    >
      <div className={cn('bg-muted/20 flex size-10 items-center justify-center rounded-lg border')}>{item.icon && <item.icon />}</div>
